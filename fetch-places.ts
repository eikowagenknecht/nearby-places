...
const PLACE_TYPES = ["restaurant", "bar", "cafe", "bakery", "ice_cream_shop"];
...
async function searchNearbyRecursive(
  ...
  const allResults: PlaceBasic[] = [...results];
...
async function main() {
  ...
  // Step 4: Filter to only places within requested radius from origin
